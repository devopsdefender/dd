...
## v2 ownership prototype

This branch keeps the existing v1 runtime intact and adds a separate
`crates/dd-agent` prototype for the v2 ownership model:

- every agent has one current GitHub owner principal;
- `/owner` is idempotent runtime assignment by an assignment authority;
- `/deploy`, `/logs/{app}`, and `/exec` are current-owner-only;
- `/health` is a public proof document.

The prototype is intentionally not wired into the v1 release/deploy workflows
yet. It exists to validate the ownership and external-repo deploy path before
porting TDX quote minting, EasyEnclave execution, Cloudflare ingress, and
managed reassignment reconciliation.
